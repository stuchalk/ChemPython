...
# with open('listfile.json', 'r', encoding="utf8") as filehandle:
...
with open('listfile.json', 'r') as filehandle:
    ...
    """Generate a list of dictionaries containing the InChiKey, heavy atom count, and carbon atoms for all entries in the identifier_list of type csmiles"""
    ...
    """Generate a list of dictionaries containing pairs of molecules and their similarity score as calculated from their fingerprint"""
...
with open('listfile.json', 'r') as filehandle:
    ...
    #         #     "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/smiles/" + entry['value'] + "/property/HBondDonorCount/txt")
    #         # HBDs = HBD_query.text
    #         carbon_atoms = len(mol.GetSubstructMatches(SMARTS_carbon_atom))
    #         molwt = Chem.Descriptors.MolWt(mol)
    #         fp = Chem.RDKFingerprint(mol)
    #         csmiles_fingerprint.append(
    #             {"csmiles": entry['value'], "fingerprint": fp, "mol2_molweight": molwt, "mol2_carbon_atoms": carbon_atoms})
    ...
    #
    # with open('fingerprintfile.json', 'w') as filehandle:
    #     json.dump(matches, filehandle, default=str)
